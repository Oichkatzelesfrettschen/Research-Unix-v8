...
static size_t dsize = 77U * 26 * 128; /* default size in bytes */
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, "d:hrt:H", long_opts, NULL)) != -1) {
    ...
    case 't': {
      ...
      char *endp = NULL;
      errno = 0;
      tracks = strtoul(optarg, &endp, 10);
      if (errno != 0 || endp == optarg || *endp != '\0' || tracks == 0 ||
          tracks > 77) {
        fprintf(stderr, "Bad number of tracks\n");
        return 2;
      }
      dsize = (size_t)tracks * 26 * 128;
      break;
    }
    default:
      usage(argv[0]);
...
static void usage(const char *progname) {
  fprintf(stderr, "Usage: %s [-d device] [-h] [-r] [-t tracks]\n", progname);
}
